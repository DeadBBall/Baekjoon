...
/*
 * 방문처리를 하는 visit배열을 3차원 배열로 만들어서 벽을 부순 경우의 방문처리와 벽을 부수지 않은 경우의 방문처리를 함께 해서 bfs로 가장 빨리 도착한 경우의 거리를 측정해준다.
 */
	...
	static int moveAndCrash() {
		...
		while (!q.isEmpty()) {
			...
			for(int i = 0; i < 4; i++) {
				...
				// 벽이 있는 곳은 벽이 있고 내가 벽을 부순 적이 없으며 벽을 부수고 방문한 적이 없다면 큐에 이동한 정보를 넣어준다.
